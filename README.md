...
corepack pnpm run build:lib # dist: React package
corepack pnpm run build     # app-dist: standalone HTML app
```

```tsx
import { Zen, type ZenRuntimeHost } from "@glink25/zen";
import "@glink25/zen/styles.css";

<Zen host={host} onClose={() => setOpen(false)} />;
```

`react`, `react-dom`, `zod` and `@glink25/chaty` are peer dependencies in the
library build. The HTML build is self-contained.
